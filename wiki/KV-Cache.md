...
Manus 认为 **KV-Cache 命中率是生产阶段 AI Agent 最重要的指标**，直接关联延迟和成本。

## 原因

Agent 与聊天机器人的关键区别：
- Agent 上下文随每个工具调用而增长，输出却很短（结构化函数调用）
- Manus 的平均输入输出 token 比例约 **100:1**
- 前缀相同的上下文可复用 KV-Cache，极大降低 TTFT（Time‑To‑First‑Token）和推理成本

## 成本差异举例

- Claude Sonnet：缓存的输入 token 0.30 USD/MTok，未缓存的 3 USD/MTok（**10 倍** 差距）

## 上下文工程中的 KV-Cache 优化实践

1. **保持提示前缀稳定** — LLM 的自回归特性导致单个 token 差异就会使后续缓存失效。不要在系统提示开头放时间戳（尤其秒级）。
