...
[[GPU Memory Math for LLMs (2026 Edition)]] 强调权重只是显存账单的一部分。长上下文、batching 和 agent workload 会让 KV-Cache 快速增长，即使 [[LLM VRAM Sizing]] 显示权重能装下，实际运行仍可能 out of memory。

[[Inference Engines for LLMs & Local AI Hardware (2026 Edition)]] 则把 KV cache 管理放进 [[LLM Inference Engines]] 的核心职责：PagedAttention、prefix caching、KV quantization、cache paging 和 prefill/decode disaggregation 都是在管理 KV cache 的容量和带宽压力。
