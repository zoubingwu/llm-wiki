...
Tailscale 是基于 [[WireGuard]] 的组网产品，把用户设备组织成一个私有 [[Tailnet]]。它把密钥分发、身份、ACL、设备发现和路径选择放在控制面，把数据传输尽量保留在设备之间的加密通道中。
...
Tailscale 的便利性来自控制面集中管理和客户端自动路径选择。运维侧仍需要处理底层网络约束，例如 UDP 端口可达性、云安全组、系统 DNS、ACL tag 授权和 relay 节点的带宽成本。
