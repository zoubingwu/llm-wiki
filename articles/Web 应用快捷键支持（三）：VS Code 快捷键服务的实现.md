...
source: "https://rebornix.com/vscode/2019/09/11/web-keyboard-support-3/"
author:
  - "[[Peng Lyu]]"
published: 2019-09-11
created: 2026-04-23
description: "这篇文章是系列文章《Web 应用快捷键支持》的第三篇。前两篇文章中介绍了如何正确处理浏览器 Keyboard Event。今天我们一起来看看，如何把正确解析的键盘事件，映射到对应的命令上。"
tags:
  - "articles"
---
...
简单的 Map 就能够避免每次都进行暴力搜索了，而且在这几百个快捷键的 scale 下，Map 足够好了。
...
VS Code 在使用了这个方法，不过却是隐式的。VS Code 在启动时，会先注册默认的快捷键，接着是加载插件注册的快捷键，最后加载用户修改的快捷键。然后遵循后注册优先级更高的方式进行覆盖。所以，如果用户将 `cmd+/` 绑定给了其他的命令，那么 Toggle Line Comment 就不会被执行了。
...
第三步，支持与操作 `&&` 和 `||` 。在解析这两个操作符的同时，要注意优先级，通常是 `&&` 要优先于 `||` 。
